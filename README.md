# PZ Config Panel

Private V1 web panel for one vanilla Project Zomboid B42 server on the same Ubuntu host.

This release edits the existing config files generated by the server:

- `<PZ_SERVER_NAME>.ini`
- `<PZ_SERVER_NAME>_SandboxVars.lua`

It does not expose the panel publicly, back up worlds, update the game server, or add full player administration yet.

## Current Baseline

Verified VPS baseline on May 21, 2026:

| Surface | Current value |
|---|---|
| Game install | `/home/steam/pz_server` |
| Game service | `project-zomboid.service` |
| Game server profile | `servertest` |
| Game config directory | `/home/steam/Zomboid/Server` |
| Panel deploy path | `/opt/pz-config-panel` |
| Panel service | `pz-config-panel.service` |
| Panel env file | `/etc/pz-config-panel.env` |
| Panel bind target | `0.0.0.0:3210` |

## What It Does

- Shows every existing setting parsed from `server.ini` and `SandboxVars.lua`.
- Categorizes settings into labelled groups (General, PvP, Network, Safehouses, Zombie Lore, XP Multipliers, etc.) with full descriptions sourced from the PZ wiki.
- Uses known controls for common booleans, numbers, enums, and sensitive fields.
- Keeps unknown settings visible with auto-generated descriptive labels.
- Saves only existing keys after a diff review.
- Masks password-like values on read.
- Requires login and refuses to start without `PANEL_PASSWORD`.
- Installs a Workshop mod from a Steam Workshop URL or ID by downloading with SteamCMD, inspecting local Mod IDs/dependencies/map folders, saving the reviewed config additions, and restarting the server when dependencies are resolved.
- Keeps the existing manual `WorkshopItems` and `Mods` list editor as an advanced fallback.
- Manages spawn regions through a dedicated Spawn tab — reads `<server>_spawnregions.lua`, shows all regions with checkboxes, rewrites the file with only enabled regions to control which spawn options appear in-game.
- Adds a Live tab backed by local RCON for online players, manual runtime commands, world save, and broadcasts.
- Creates per-file backups before saves and restores, with last-20 retention.
- Uses revision checks so an old browser view cannot overwrite a newer file.
- Shows `Online` when the configured `systemd` unit is `active / running`, otherwise shows `Offline`, and asks for confirmation before restart.
- Separates saved disk config from loaded/runtime evidence: `server.ini` can be applied and compared through RCON, while Sandbox, Mods, and Spawn use service-restart evidence.

V1 reads and writes config files on disk. After a save, the application status block shows whether a file is only saved, is newer than the running service, was loaded after a service restart, or matches the comparable `server.ini` options exposed by RCON. V1 does not yet compare disk config against sandbox values loaded inside the running game process.

## Local Run

Requirements:

- Node.js 18 or newer
- Existing Project Zomboid B42 config files for the configured server profile

Install dependencies, export local environment values, and start the localhost panel:

```bash
npm install
cp .env.example .env
# Edit .env for the local or VPS paths you are testing.
set -a
. ./.env
set +a
npm start
```

Open `http://127.0.0.1:3210`.

The server reads settings from environment variables. Defaults are localhost-only:

| Variable | Default | Purpose |
|---|---|---|
| `PANEL_HOST` | `0.0.0.0` | Panel bind address |
| `PANEL_PORT` | `3210` | Panel HTTP port |
| `PZ_CONFIG_DIR` | `/home/steam/Zomboid/Server` | Directory containing PZ config files |
| `PZ_SERVER_DIR` | `/home/steam/pz_server` | Dedicated server install directory used for Workshop downloads and local mod inspection |
| `PZ_STEAMCMD_PATH` | `/usr/games/steamcmd` | Executable SteamCMD path used for anonymous Workshop item downloads |
| `PZ_SERVER_NAME` | `servertest` | Single server profile name |
| `PZ_PANEL_BACKUP_DIR` | `<PZ_CONFIG_DIR>/.pz-config-panel-backups` | Config backup directory |
| `PZ_SYSTEMD_UNIT` | `project-zomboid.service` | PZ server system unit to inspect and restart |
| `PZ_RCON_HOST` | `127.0.0.1` | Local RCON host used by the Live tab |
| `PZ_RCON_PORT` | `27015` | Local RCON port used by the Live tab |
| `PZ_RCON_PASSWORD` | *(unset)* | RCON password required for Live operations |
| `PZ_RCON_TIMEOUT_MS` | `5000` | RCON connect/response timeout |
| `PANEL_USERNAME` | `admin` fallback with warning | Login username |
| `PANEL_PASSWORD` | *(required)* | Login password |
| `SESSION_TTL_HOURS` | `8` | Bearer token session lifetime in hours |

The panel requires a login. All API routes are protected with a Bearer token session except the login endpoint. `PANEL_PASSWORD` is required at startup; if `PANEL_USERNAME` is omitted, V1 logs a warning and falls back to `admin`.

## VPS Deployment

The deployment examples live in [`deploy/`](deploy/).

1. Create `/opt/pz-config-panel` on the VPS and make it owned by `steam`.
2. Upload `package.json`, `package-lock.json`, `src/`, `public/`, `deploy/`, and `README.md` into that directory.
3. Install Node dependencies there with `sudo -u steam npm install --omit=dev`.
4. Copy `deploy/pz-config-panel.env.example` to `/etc/pz-config-panel.env` and adjust the config path, server install path, SteamCMD path, server profile name, and set a strong `PANEL_PASSWORD`.
5. Install `deploy/project-zomboid.service` as `/etc/systemd/system/project-zomboid.service` if the current vanilla server is not already managed by a system service. Adjust `WorkingDirectory` and `ExecStart` if the server startup command differs.
6. Install `deploy/pz-config-panel.service` as `/etc/systemd/system/pz-config-panel.service`.
7. Install `deploy/pz-config-panel.sudoers` as `/etc/sudoers.d/pz-config-panel` with mode `0440`, then validate it with `visudo -cf`.
8. Reload and enable the services:

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now project-zomboid.service
sudo systemctl enable --now pz-config-panel.service
```

The panel status check uses `systemctl show`. The restart action runs only:

```bash
sudo -n systemctl restart project-zomboid.service
```

The provided sudoers snippet grants that restart path to `steam` and nothing broader. Do not set `NoNewPrivileges=true` on the panel unit while this V1 restart path uses `sudo`; systemd will block that sudo escalation before sudoers can apply.

The Workshop installer runs SteamCMD as the panel user with anonymous login. `PZ_SERVER_DIR` must be writable by that user and the downloaded content is expected under `PZ_SERVER_DIR/steamapps/workshop/content/108600`. The install flow inspects local `mod.info` `require=` values; when a required Mod ID cannot be resolved from downloaded Workshop content it asks for that dependency Workshop URL or ID before it will save or restart.

The Live tab uses RCON separately from the panel login. Set `RCONPort=27015` and a strong `RCONPassword` in `servertest.ini`, copy the same password into `/etc/pz-config-panel.env` as `PZ_RCON_PASSWORD`, and restart `project-zomboid.service` before expecting RCON to be ready. Keep RCON on `127.0.0.1`; do not open `27015/tcp` publicly for this direct-host panel.

Useful VPS checks:

```bash
sudo systemctl status project-zomboid.service --no-pager
sudo systemctl status pz-config-panel.service --no-pager
# API calls need a token:
curl -H 'Authorization: Bearer <token>' http://127.0.0.1:3210/api/server/status
```

## Access

The panel binds to `0.0.0.0` by default so you can reach it at `http://your-vps-ip:3210` directly. A login page protects the panel — set `PANEL_USERNAME` and `PANEL_PASSWORD` in the environment.

For an additional layer, you can reverse-proxy behind nginx with HTTPS or use Cloudflare Tunnel. The SSH tunnel method still works when you prefer not to open the port at all:

```bash
ssh -L 3210:127.0.0.1:3210 your-user@your-vps
```

Then open `http://127.0.0.1:3210` locally.

Opening Project Zomboid game ports or configuring VPS/firewall port forwarding is separate from this panel.

## API

The UI uses the same-origin JSON API. All endpoints except `POST /api/auth/login` require a Bearer token obtained from login:

- `POST /api/auth/login` — authenticate, returns `{ token }`
- `POST /api/auth/logout` — invalidate session
- `GET /api/auth/check` — verify token is still valid
- `GET /api/config/ini`
- `PATCH /api/config/ini`
- `POST /api/config/ini/apply`
- `GET /api/config/sandbox`
- `PATCH /api/config/sandbox`
- `GET /api/config/apply-status?surface=ini|sandbox|mods|spawn`
- `GET /api/mods`
- `PATCH /api/mods`
- `POST /api/mods/install`
- `GET /api/mods/install/:operationId`
- `POST /api/mods/install/:operationId/dependencies`
- `POST /api/mods/install/:operationId/apply`
- `GET /api/config/spawn`
- `PATCH /api/config/spawn`
- `GET /api/config/backups?file=ini|sandbox`
- `POST /api/config/backups/:backupId/restore`
- `GET /api/server/status`
- `POST /api/server/restart`
- `GET /api/live/rcon/status`
- `GET /api/live/players`
- `POST /api/live/commands`
- `POST /api/live/save`
- `POST /api/live/broadcast`

Save requests contain:

```json
{
  "revision": "sha256-of-loaded-file",
  "changes": {
    "MaxPlayers": "24"
  }
}
```

Only changed keys already present in the source file are accepted. The dedicated mods API still requires the `WorkshopItems` and `Mods` keys to already exist in `server.ini`; it writes those lists together with the same INI revision and backup protections. A reviewed Workshop install can add detected map folders to an existing `Map` line in that same guarded mods save.

The spawn API reads and rewrites `<PZ_SERVER_NAME>_spawnregions.lua` entirely. The payload contains the full regions array with `enabled` booleans; disabled regions are excluded from the rewritten file. If the file does not exist, the panel returns 4 vanilla region defaults and creates the file on first save.

`POST /api/config/ini/apply` takes the loaded INI revision, runs RCON `reloadoptions`, then compares comparable non-sensitive `server.ini` values against RCON `showoptions`. It rejects stale revisions before touching runtime state. `GET /api/config/apply-status` reports disk revision timing against the configured `systemd` active timestamp for the requested editor surface.

## Sandbox Verification Boundary

For a saved sandbox change, V1 can prove:

1. The diff was accepted for an existing `SandboxVars.lua` key.
2. The file on disk contains the saved value.
3. A backup exists.
4. Whether the configured game service restarted after that disk revision was written.

V1 cannot yet prove the running game process is using that exact sandbox value. The `server.ini` Apply live path is narrower: RCON `reloadoptions` plus `showoptions` can compare only the non-sensitive INI options that RCON exposes. A future runtime verification path for Sandbox should use an in-game/RCON/bridge surface; the `zomboid-control-panel` reference under `repos/` includes a `PanelBridge` sandbox introspection path worth evaluating.

The Live tab is a separate runtime surface: RCON can show players and execute server commands while the disk editor continues to show saved config file state.

## Verification

Run the automated test suite:

```bash
npm test
```

Manual VPS smoke test:

1. Start the B42 server once so the profile config files exist.
2. Point `/etc/pz-config-panel.env` at that `Server` directory and profile name.
3. Reach the panel at `http://<vps-ip>:3210` and log in.
4. Collapse and search config groups, then change a harmless existing setting, review the diff, and save.
5. For a harmless `server.ini` setting, use `Apply live` and confirm the application state reports the RCON comparison result.
6. Open the Mods tab, run a Workshop installer dry smoke on a known mod URL through review, or review a manual Workshop ID or Mod ID list change and save only when the INI edit is intended.
7. Open the Spawn tab, uncheck one or more regions, review the diff, and save. Verify that `<server>_spawnregions.lua` on disk contains only the enabled regions.
8. Confirm a backup appears in the backup rail.
9. Restore that backup and confirm the file value returns.
10. Confirm status is visible and restart requires confirmation; Sandbox, Mods, and Spawn show restart evidence instead of a live apply claim.
11. Configure local RCON, open the Live tab, refresh players, run a safe command, send a broadcast, and confirm the response history stays in the browser session.
